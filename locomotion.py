...
class LocomotionControl:
    ...
    def __init__(
        ...
        params: np.ndarray = np.ones(len(cp_keys)),
    ):
    ...
    def reset(self, params=None):
        ...
        self.balance_stim["r_leg"] = dict(
            zip(self.m_keys, 0.01 * np.ones(len(self.m_keys)))
        )
        self.reflex_stim["r_leg"] = dict(
            zip(self.m_keys, 0.01 * np.ones(len(self.m_keys)))
        )

        self.stim["l_leg"] = dict(zip(self.m_keys, 0.01 * np.ones(len(self.m_keys))))
        self.balance_stim["l_leg"] = dict(
            zip(self.m_keys, 0.01 * np.ones(len(self.m_keys)))
        )
        self.reflex_stim["l_leg"] = dict(
            zip(self.m_keys, 0.01 * np.ones(len(self.m_keys)))
        )
    ...
    def _brain_control(self, sensor_data):
    ...
    def _spinal_control(self, sensor_data):
        for s_leg in ["r_leg", "l_leg"]:
            ...
            self.stim[s_leg], self.balance_stim[s_leg], self.reflex_stim[s_leg] = (
                self.spinal_control_leg(s_leg, sensor_data)
            )
...
class ReflexLocomotionControl(LocomotionControl):
    """
    adapted from:
    - Song and Geyer. "A neural circuitry that emphasizes spinal feedback generates
    diverse behaviours of human locomotion."
    The Journal of physiology, 2015.
    """
    ...
    def __init__(
        ...
        num_walking_parameters = len(individual)

        self.par_space = (
            FB_PAR_SPACE_3D[0][0:num_walking_parameters],
            FB_PAR_SPACE_3D[1][0:num_walking_parameters],
        )

        super().__init__(
            control_dimension=control_dimension,
            params=np.ones(self.n_par),
        )

        individual = self._scale_feedback(np.array(individual))
        control_params = np.round(individual[0:num_walking_parameters], 4)
        self.set_control_params(control_params)

    def _scale_feedback(self, feedback: np.ndarray) -> np.ndarray:
        lower, upper = np.array(self.par_space[0]), np.array(self.par_space[1])
        return lower + (upper - lower) * (feedback / 10)
    ...
    def _obs2reflexobs(self, observation):
        ...
        body = {
            ...
            "d_pos": [
                # local x (+) forward
                observation["pelvis"]["vel"][0],
                # local y (+) leftward
                observation["pelvis"]["vel"][1],
            ],
            "dtheta": [
                # around local x axis
                observation["pelvis"]["vel"][4],
                # around local y axis
                observation["pelvis"]["vel"][3],
            ],
        }

        legs = {}
        for leg in ["r_leg", "l_leg"]:
            opposite_leg = "r_leg" if leg == "l_leg" else "l_leg"

            load_ipsi = observation[leg]["ground_reaction_forces"][2]
            opposite_load_ipsi = observation[opposite_leg]["ground_reaction_forces"][2]

            obs = observation[leg]

            phi_hip = obs["joint"]["hip"] + np.pi
            dphi_hip = obs["d_joint"]["hip"]
            phi_knee = obs["joint"]["knee"] + np.pi
            dphi_knee = obs["d_joint"]["knee"]

            legs[leg] = {
                "load_ipsi": load_ipsi,
                "contact_ipsi": 1 if load_ipsi > 0.1 else 0,
                "contact_contra": 1 if opposite_load_ipsi > 0.1 else 0,
                "load_contra": opposite_load_ipsi,
                "phi_hip": phi_hip,
                "phi_knee": phi_knee,
                "phi_ankle": obs["joint"]["ankle"] + 0.5 * np.pi,
                "dphi_knee": obs["d_joint"]["knee"],
                "alpha": phi_hip - 0.5 * phi_knee,
                "dalpha": dphi_hip - 0.5 * dphi_knee,
                "alpha_f": -obs["d_joint"]["hip_abd"] + 0.5 * np.pi,
                "F_RF": obs["RF"]["f"],
                "F_VAS": obs["VAS"]["f"],
                "F_GAS": obs["GAS"]["f"],
                "F_SOL": obs["SOL"]["f"],
        ...
        return {"body": body, **legs}

    def _stimdict2array(self, _stm, balance=True):
        stim = [
            _stm["r_leg"]["HFL"],  # (iliopsoas_r)
        ...
        elif self.mode == "2D" and balance:
            Fmax_ABD = 4460.290481
            Fmax_ADD = 3931.8

            stim.insert(0, 0.1)
            stim.insert(1, 0.1 * Fmax_ADD / Fmax_ABD)
            stim.insert(11, 0.1)
            stim.insert(12, 0.1 * Fmax_ADD / Fmax_ABD)
