...
class OsimModel(object):
    ...
    def __init__(self, model_path, exoskeleton, visualize, integrator_accuracy=5e-5):
    ...
    def actuate(self, action):
        ...
        self.last_action = np.clip(np.array(action[0:21]), 0.0, 1.0)  # muscle actions
        if self.exoskeleton:
            action_actuators = np.clip(np.array(action[21:]), -10.0, 10.0)
            self.last_action = np.concatenate((self.last_action, action_actuators))
    ...
    def get_activations(self):
        return [
            self.muscleSet.get(j).getActivation(self.state)
            for j in range(self.muscleSet.getSize())
        ]
    ...
    def set_state(self, state):
        self.state = state
        self.istep = int(
            self.state.getTime() / self.stepsize
        )  # TODO: remove istep altogether
        self.reset_manager()
...
class OsimEnv(gym.Env):
    ...
    def __init__(self, exoskeleton, visualize, integrator_accuracy):
        self.exoskeleton = exoskeleton
    ...
    def load_model(self, model_path=None):
        ...
        self.action_space = (
            [0.0] * self.osim_model.get_action_space_size(),
...
class L2M2019Env(OsimEnv):
    ...
    # -------------------------------- SKIPPED -----------------------------
    model = "3D"  # what to do with the 3D part if we are only working in 2D? Need to adapt the code?
    ...
    obs_body_space[:, [9 + x for x in [0, 44]]] = np.array([[-5, 5]]).transpose()
    # (r,l) ground reaction force normalized to bodyweight (forward)
    obs_body_space[:, [10 + x for x in [0, 44]]] = np.array(
        [[-5, 5]]
    ).transpose()  # (r, l) ground reaction force normalized to bodyweight (rightward)
    obs_body_space[:, [11 + x for x in [0, 44]]] = np.array(
        [[-10, 10]]
    ).transpose()  # (r, l) ground reaction force normalized to bodyweight (upward)
    obs_body_space[:, [12 + x for x in [0, 44]]] = np.array(
        [[-45 * np.pi / 180, 90 * np.pi / 180]]
    ).transpose()  # (r, l) joint: (+) hip abduction
    obs_body_space[:, [13 + x for x in [0, 44]]] = np.array(
        [[-180 * np.pi / 180, 45 * np.pi / 180]]
    ).transpose()  # (r, l) joint: (+) hip extension
    obs_body_space[:, [14 + x for x in [0, 44]]] = np.array(
        [[-180 * np.pi / 180, 15 * np.pi / 180]]
    ).transpose()  # (r, l) joint: (+) knee extension
    ...
    obs_body_space[
        ...
        [[-50, 50]]
    ).transpose()  # (r, l) muscle velocities, normalized to optimal length per second
    ...
    def __init__(
        ...
        model_path = Path(model_path).absolute().__str__()
        self.model_paths = {"2D": model_path, "3D": model_path}
        self.model_path = self.model_paths[self.get_model_key()]

        self.act2mus = ACT2MUS
        if not exoskeleton:
            self.act2mus = self.act2mus[:22]

        super(L2M2019Env, self).__init__(
        ...
        self.vtgt = VTgtField(
            visualize=visualize, version=self.difficulty, dt=self.osim_model.stepsize
        )
        self.obs_vtgt_space = self.vtgt.vtgt_space

        state = self.osim_model.model.updWorkingState()
        self.MASS = self.osim_model.model.getTotalMass(state)
    ...
    def update_footstep(self):
        ...
        r_contact = (
            True
            if state_desc["forces"]["foot_r"][1] < -0.05 * (self.MASS * self.G)
            else False
        )
        l_contact = (
            True
            if state_desc["forces"]["foot_l"][1] < -0.05 * (self.MASS * self.G)
            else False
        )
    ...
    def get_observation_list_joints_pos(self):
        state_desc = self.get_state_desc()
        hip_joint_pos = [
            -state_desc["joint_pos"]["hip_{}".format("r")][0],
            -state_desc["joint_pos"]["hip_{}".format("l")][0],
        ]
        knee_joint_pos = [
            state_desc["joint_pos"]["knee_{}".format("r")][0],
            state_desc["joint_pos"]["knee_{}".format("l")][0],
        ]
        ankle_joint_pos = [
            -state_desc["joint_pos"]["ankle_{}".format("r")][0],
            -state_desc["joint_pos"]["ankle_{}".format("l")][0],
        ]
    ...
    def get_observation_list_joints_vel(self):
        state_desc = self.get_state_desc()
        hip_joint_vel = [
            -state_desc["joint_vel"]["hip_{}".format("r")][0],
            -state_desc["joint_vel"]["hip_{}".format("l")][0],
        ]
        knee_joint_vel = [
            state_desc["joint_vel"]["knee_{}".format("r")][0],
            state_desc["joint_vel"]["knee_{}".format("l")][0],
        ]
        ankle_joint_vel = [
            -state_desc["joint_vel"]["ankle_{}".format("r")][0],
            -state_desc["joint_vel"]["ankle_{}".format("l")][0],
        ]
    ...
    def get_observation_dict(self):
        ...
        obs_dict["pelvis"]["pitch"] = -state_desc["joint_pos"]["ground_pelvis"][
            0
        ]  # (+) pitching forward
        ...
        for leg, side in zip(["r_leg", "l_leg"], ["r", "l"]):
            ...
            for MUS, mus in zip(
                ...
                obs_dict[leg][MUS] = {}
                obs_dict[leg][MUS]["f"] = (
                    state_desc["muscles"]["{}_{}".format(mus, side)]["fiber_force"]
                    / self.Fmax[leg][MUS]
                )
                obs_dict[leg][MUS]["l"] = (
                    state_desc["muscles"]["{}_{}".format(mus, side)]["fiber_length"]
                    / self.lopt[leg][MUS]
                )
                obs_dict[leg][MUS]["v"] = (
                    state_desc["muscles"]["{}_{}".format(mus, side)]["fiber_velocity"]
                    / self.lopt[leg][MUS]
                )
    ...
    def get_reward_1(self):  # for L2M2019 Round 1
        ...
        if self.footstep["new"]:
            ...
            reward_footstep_0 = (
                self.d_reward["weight"]["footstep"] * self.d_reward["footstep"]["del_t"]
            )
            ...
            # penalize effort
            reward_footstep_e = (
                -self.d_reward["weight"]["effort"] * self.d_reward["footstep"]["effort"]
            )
        ...
        reward_velocity = (
            1 - ((2 * mean_squared_error(v_body, v_tgt)) / (v_body + v_tgt)) * dt
        )
