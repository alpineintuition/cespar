...
class OsimModel(object):
    ...
    def __init__(self, model_path, visualize, integrator_accuracy=5e-4):
        ...
        func = opensim.Constant(1.0)
        self.brain.addActuator(
            self.model.getActuators().get("exo_hip_motor_flex_right")
        )
...
class L2M2019Env(OsimEnv):
    ...
    obs_body_space[:, [9 + x for x in [0, 44]]] = np.array(
        [[-5, 5]]
    ).transpose()  # (r,l) ground reaction force normalized to bodyweight (forward)
    ...
    def __init__(
        ...
        if __main__.__file__ == "model_generator.py":
            self.model_paths["2D"] = os.path.join(
                os.path.dirname(__file__), "../models/gait14dof22musc_withexo_new.osim"
            )
            ...
            self.model_paths["2D"] = os.path.join(
                os.path.dirname(__file__), "../models/gait14dof22musc_withexo_full.osim"
            )
            self.model_paths["3D"] = os.path.join(
                os.path.dirname(__file__), "../models/gait14dof22musc_withexo_full.osim"
            )
